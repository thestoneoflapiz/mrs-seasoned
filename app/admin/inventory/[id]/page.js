...
import EditInventoryModal from "../_modals/edit";
import DeleteInventoryModal from "../_modals/delete";

export default function InventoryItem({ params }){
  ...
  function getInventoryItem(){
    ...
    console.log("Inventory DATA HERE");
  }

  return (
    <>
      <main className={styles.main}>
        <Header pageTitle="Mrs. Seasoned - Inventory" activePage="Inventory"/>
        <Container>
          <div className={styles.c_div}>
            <Row>
              <Col>
                <h3 className="text-light">Inventory - Item</h3>
                <Breadcrumb>
                  <Breadcrumb.Item className={styles.c_link} href="/admin">Home</Breadcrumb.Item>
                  <Breadcrumb.Item className={styles.c_link} href="/admin/inventory">Inventory</Breadcrumb.Item>
          ...
          <div className={styles.c_div__color}>
            ...
            <Row>
              {data ? <Datatable dataList={data} pageLink="/admin/inventory"/> : <h3>No Record Found.</h3>}
            </Row>
          </div>

          <EditInventoryModal show={showEditModal} onModalClose={handleEditModal} data={itemDetails}/>
          <DeleteInventoryModal show={showDeleteModal} onModalClose={handleDeleteModal} data={itemDetails}/>
        </Container>
      </main>
    </>
  )
