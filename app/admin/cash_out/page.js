...
import AddCOModal from "./_modals/add";
...
export default function COPage(){
  ...
  const [data, setData] = useState({
    list: [],
    headers: ["Reason","Amount","Created"],
    keys: ["reason","amount", "created"],
    sortable: ["reason", "amount"],
    limit: 10,
    page: 1,
    pages: 0,
    total: 0,
  ...
  async function getCO(){
    ...
    const response = await fetch(`/api/cash_out/list?${filters}`, {
      method: "GET",
    });
    ...
    <> 
      <Container className="pb-5">
        ...
        <div className={styles.c_div}>
          <Row>
            <Col>
              <h3 className="text-light">Cash OUT</h3>
              <Breadcrumb>
                <Breadcrumb.Item className={styles.c_link} href="/admin">Home</Breadcrumb.Item>
                <Breadcrumb.Item className={styles.c_link} active>Cash OUT</Breadcrumb.Item>
              </Breadcrumb>
            </Col>
          </Row>
      ...
      <AddCOModal show={showAddModal} onModalClose={handleModalClose} />
    </>
  );
}